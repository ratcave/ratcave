...
class TestPhysical(unittest.TestCase):
    ...
    """
    def test_child_update_matrix_function(self):
        for i in range(100):
    ...
    def test_add_child_connection_modify_scale(self):
        # N = P * O
        for _ in range(200):
            child =  PhysicalGraph(position=np.random.randint(0, 360, 3), rotation=np.random.randint(0, 360, 3), scale=np.random.randint(0, 360))
            parent = PhysicalGraph(position=np.random.randint(0, 360, 3), rotation=np.random.randint(0, 360, 3), scale=np.random.randint(0, 360))
